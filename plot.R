...
group.row.verification <- function(xtest){
  # verification of groups vs rows
  print("*******************+verification of groups vs rows*******************+++")
  last.group = -1
  print(paste(" first group at ",1))
  for (i in 1:nrow(xtest)){
    newgroup = xtest$group[i]
    if (newgroup!=last.group){
      print(paste(" new group at ",i))
...
sop.plot.prepare <- function(xtest,ytest, ytest.pred, raw=FALSE, dp){
  ...
  desired_length <- nrow(xtest)
  
  #group.row.verification(xtest)
  ...
  for (i in 1:nrow(xtest.backup)){
    # this for loop get every line
    # when row is the first row of a new group("experiment)
    # it adds the
    
    group <- xtest.backup$group[i]
    if (current.group != group){
      #print(paste("******************new group*******************************", group, " i = ",i))
      current.group <- group
      
      #  add previous group values to result dataframe
      if (last.group.index>0){
        #print(paste("------->adding values from last rows of previous group from df rows: ",i-last.group.index+1 ))
      ...
      x1.w = as.numeric(xtest[i,(index.sop2-1):(index.sop1+1)])
      x2.w = as.numeric(xtest[i,(index.sop3-1):(index.sop2+1)])
      x3.w = as.numeric(xtest[i,(index.error):(index.sop3+1)])
      y.w = as.numeric(xtest[i,index.last.error:(index.error+1)])
      ys.w = as.numeric(xtest[i,index.last.smoothed.error:(index.smoothed.error+1)])
      ...
      if (i>1){
        ...
        #print(paste(" adding previous data (from cols)" ,"length y.w",length(y.w)))
        
        
        ytest.pred.result <- c(ytest.pred.result,rep(0,w))
        ...
        ytest.pred.result <- c(rep(0,w))
        
        #print(paste("first row/group of previous data(from cols or 0s)","length y.w",length(y.w)))
      }
      
      #print(paste("RESULT:       new dim xtest",dim(xtest.result)[1],dim(xtest.result)[2]," new dim ytest",length(ytest.result)))
  ...
  #print(paste("------>adding final normal rows from last group ",i-last.group.index+1 ))
  xtest.result <- rbind(xtest.result,xtest[last.group.index:i,])
  # smoothed error version
  ...
  # now we correct for the desired size
  print(paste("new dim xtest",dim(xtest.result)[1],dim(xtest.result)[2]," new dim ytest",length(ytest.result)))
  ytest.pred.result <- ytest.pred.result[1:desired_length]  
  ytest.result <- ytest.result[1:desired_length]
  xtest.result <- xtest.result[1:desired_length,]
  print(paste("corrected dim xtest",dim(xtest.result)[1],dim(xtest.result)[2]," corrected dim ytest",length(ytest.result)))
...
sop.plot <- function(xtest, ytest,ytest.pred,modelfilename,datafilename,rmse.plot, dsw,dp, raw=FALSE){
  ...
  result <- sop.plot.prepare(xtest,ytest,ytest.pred, raw, dp)
  ...
  # plot
  par(mfrow=c(1,1))
  xvalues = c(1:nrow(xtest))*dsw
  print(paste("dsw",dsw))
  print(paste(" xvalues first and last ",xvalues[1],xvalues[length(xvalues)]))
  ylimmax = max(max(ytest),max(ytest.pred))
  ytest.pred.margins <- ytest.pred
  ytest.pred.margins[ytest.pred.margins==0] <- max(ytest.pred.margins)
  ylimmin = min(min(ytest),min(ytest.pred.margins))
  
  print(paste("ylimmin",ylimmin,"min(ytest)",min(ytest),"min(ytest.pred.margins)",min(ytest.pred.margins) ))
  ...
  lines(
    xvalues, 
    xtest[,index.sop2], col="yellow")
  lines(
    xvalues, 
    xtest[,index.sop3], col="cyan")
  plot(
    xvalues,
    ytest,type="l", 
    main=paste("Test result",sep="",collapse=""),
